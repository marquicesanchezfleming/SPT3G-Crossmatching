...
def create_download_url(ra, dec, width=1200, height=900, fov=0.167):
    ...
    url = (f"https://alasky.cds.unistra.fr/hips-image-services/hips2fits?"
           f"hips=CDS%2FP%2FallWISE%2Fcolor&width={width}&height={height}"
...
os.makedirs('../Images/allwise_fits', exist_ok=True)
os.makedirs('../Images/allwise_cutouts', exist_ok=True)
...
for index, row in coordinates_df.iterrows():
    ...
    fits_filename = os.path.join('../Images/allwise_fits', f"{name}.fits")
    png_filename = os.path.join('../Images/allwise_cutouts', f"{name}_allwise.png")
